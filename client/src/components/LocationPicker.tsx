...
const RecenterMap = ({ position, shouldZoom }: { position: L.LatLngExpression, shouldZoom?: boolean }) => {
    const map = useMap();
    useEffect(() => {
        if (shouldZoom) {
...
export const LocationPicker: React.FC<LocationPickerProps> = ({ 
  ...
  const handleSearch = async () => {
    ...
    try {
        ...
        const response = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(address)}&countrycodes=np&addressdetails=1&limit=5`);
      ...
      <div className="relative rounded-2xl overflow-hidden border-2 border-gray-100 shadow-sm h-[400px] z-0">
         ...
         >
            ...
            <MapClickHandler 
                 ...
                 onLocationSelect={onLocationSelect}
            />
            <RecenterMap position={position} shouldZoom={isLocating || isSearching} />
         </MapContainer>
