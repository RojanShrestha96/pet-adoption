  ...
  return <AnimatePresence>
      {isOpen && <>
        ...
        backdropFilter: 'blur(4px)'
          ...
          stiffness: 400,
          damping: 30
        }} className="w-full max-w-lg pointer-events-auto flex flex-col" style={{
          maxHeight: '85vh',
          ...
          boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)'
            ...
            borderColor: 'var(--color-border)'
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                ...formData,
                date: e.target.value
              })} min={new Date().toISOString().split('T')[0]} className="w-full px-4 py-3 border-2 rounded-xl focus:outline-none transition-colors" style={{
                borderColor: 'var(--color-border)',
                background: 'var(--color-card)',
                color: 'var(--color-text)'
              }} />
                ...
                color: 'var(--color-text)'
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {timeSlots.map(slot => <button key={slot} onClick={() => setFormData({
                  ...formData,
                  time: slot
                })} className="px-3 py-2 rounded-xl text-sm font-medium transition-all" style={{
                  background: formData.time === slot ? 'var(--color-primary)' : 'var(--color-surface)',
                  color: formData.time === slot ? 'white' : 'var(--color-text)',
                  border: '2px solid',
                  borderColor: formData.time === slot ? 'var(--color-primary)' : 'transparent'
                }}>
                        {slot}
                      </button>)}
                  </div>
                ...
                color: 'var(--color-text)'
                  ...
                  <Input placeholder="Enter meeting location" value={formData.location} onChange={e => setFormData({
                ...formData,
                ...
                color: 'var(--color-text)'
                  ...
                  <textarea placeholder="Any special instructions or notes..." value={formData.notes} onChange={e => setFormData({
                ...formData,
                notes: e.target.value
              })} rows={3} className="w-full px-4 py-3 border-2 rounded-xl focus:outline-none transition-colors resize-none" style={{
                borderColor: 'var(--color-border)',
                background: 'var(--color-card)',
                color: 'var(--color-text)'
              }} />
            ...
            background: 'var(--color-card)'
                ...
                {onComplete && (
                  <Button variant="primary" className="bg-green-600 hover:bg-green-700 border-green-600" onClick={onComplete} icon={<Check className="w-4 h-4" />}>
                    Complete Meeting
                  </Button>
                )}
                <Button variant="primary" onClick={handleConfirm} disabled={!isValid} icon={<Calendar className="w-4 h-4" />}>
                  Save Changes
                </Button>
              </div>
            </motion.div>
