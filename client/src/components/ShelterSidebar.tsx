...
export function ShelterSidebar() {
  ...
  return (
    <>
      ...
      >
        {/* Logo */}
        <Link to="/shelter/dashboard" className="flex items-center gap-3 mb-10 px-2">
          <div 
              ...
              >
                <item.icon 
                  className={`w-5 h-5 transition-colors ${
                    active ? 'text-white' : 'text-gray-400 group-hover:text-[var(--color-primary)]'
                  }`} 
                />
                <span className={`font-medium ${active ? '' : 'text-gray-600 group-hover:text-gray-900'}`}>
                  {item.label}
                </span>
              </Link>
            );
