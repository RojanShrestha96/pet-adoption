...
export function AdminSheltersPage() {
  ...
  const handleApprove = (id: string) => {
    setShelters(shelters.map(s => s.id === id ? {
      ...s,
    ...
    background: 'var(--color-background)'
      ...
      <main className="flex-1 p-8">
        ...
        <div className="space-y-6">
          {shelters.map((shelter, index) => <motion.div key={shelter.id} initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
              ...
              <Card padding="lg">
                <div className="flex items-start justify-between mb-6">
                  ...
                  opacity: 0.1
                    ...
                    color: 'var(--color-text)'
                      ...
                      <div className="space-y-1 text-sm" style={{
                    color: 'var(--color-text-light)'
                  }}>
                  ...
                  <Badge variant={shelter.status === 'approved' ? 'success' : shelter.status === 'rejected' ? 'neutral' : 'warning'}>
                    {shelter.status}
                  </Badge>
                </div>
                ...
                color: 'var(--color-text)'
                  ...
                  <div className="flex gap-3">
                    {shelter.documents.map((doc, idx) => <div key={idx} className="px-4 py-2 rounded-lg text-sm" style={{
                ...
                {shelter.status === 'pending' && <div className="flex gap-3">
                    <Button variant="primary" icon={<Check className="w-4 h-4" />} onClick={() => handleApprove(shelter.id)}>
                      Approve Shelter
                    </Button>
                    <Button variant="outline" icon={<X className="w-4 h-4" />} onClick={() => handleReject(shelter.id)} style={{
