...
import { MapPinIcon, PhoneIcon, MailIcon, ClockIcon, HeartIcon, UsersIcon, PawPrintIcon, CalendarIcon, MessageSquareIcon, StarIcon, ChevronRightIcon } from 'lucide-react';
...
export function ShelterProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<'all' | 'dogs' | 'cats' | 'other'>('all');
  ...
  // Filter pets by type
  const filteredPets = activeTab === 'all' ? shelterPets : shelterPets.filter(pet => {
    if (activeTab === 'dogs') return (pet.species as string).toLowerCase() === 'dog';
    if (activeTab === 'cats') return (pet.species as string).toLowerCase() === 'cat';
    return (pet.species as string).toLowerCase() !== 'dog' && (pet.species as string).toLowerCase() !== 'cat';
  });
  ...
  if (!shelter) {
      return (
        <div className="min-h-screen bg-[var(--color-background)] flex flex-col items-center justify-center gap-4">
             <h2 className="text-2xl font-bold" style={{ color: 'var(--color-text)' }}>Shelter not found</h2>
             <Link to="/">
  ...
  const stats = [
    { label: 'Pets Available', value: shelter.stats?.totalPets || 12, icon: <PawPrintIcon className="w-5 h-5" /> },
    { label: 'Pets Adopted', value: shelter.stats?.adoptions || 85, icon: <HeartIcon className="w-5 h-5" /> },
    { label: 'Volunteers', value: 24, icon: <UsersIcon className="w-5 h-5" /> }, // Mock for now
    { label: 'Rating', value: shelter.rating || 'New', icon: <StarIcon className="w-5 h-5" /> }
  ];
      ...
      <main className="pt-20 pb-16">
        ...
        >
          ...
          <div className="absolute inset-0">
            <img 
              src={shelter.coverImage || "https://images.unsplash.com/photo-1450778869180-41d0601e046e?w=1200&h=400&fit=crop"} 
              alt="Cover" 
          ...
          <motion.div variants={containerVariants} initial="hidden" animate="visible">
                  ...
                  <div className="flex-1 text-center lg:text-left">
                    <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                      <div>
                        ...
                        <div className="flex items-center justify-center lg:justify-start gap-2 mb-4" style={{ color: 'var(--color-text-light)' }}>
                          <MapPinIcon className="w-4 h-4 text-[var(--color-primary)]" />
                          <span>{displayAddress}</span>
                        </div>

                        {/* Contact Info */}
                        <div className="flex flex-wrap justify-center lg:justify-start gap-4 text-sm" style={{ color: 'var(--color-text-light)' }}>
                          {shelter.phone && (
                          ...
                          <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-[var(--color-surface)] border border-[var(--color-border)]">
                            {/* Quick Status: Open/Closed */
                             (() => {
                                if (!shelter.operatingHours || typeof shelter.operatingHours === 'string') return <span className="text-gray-500">Hours N/A</span>;
                                ...
                                if (!status || status.closed) {
                                  return (
                                    <>
                                      ...
                                      <span className="font-medium text-red-600">Closed Today</span>
                                    </>
                                  );
                                }
                                return (
                                  <>
                                    ...
                                    <span className="font-medium text-green-700">Open Today: {formatTime(status.open)} - {formatTime(status.close)}</span>
                                  </>
                                );
                             })()
                            }
                            ...
                            onClick={async () => {
                              ...
                              try {
                                const res = await fetch('http://localhost:5000/api/messages', {
                                    ...
                                    body: JSON.stringify({ recipientId: shelter._id || shelter.id }) 
                                });
                                ...
                                console.error(err);
                                if (shelter.email) window.location.href = `mailto:${shelter.email}`;
                              }
                            }}
                        >
            ...
            <motion.div variants={itemVariants} className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              {stats.map((stat, index) => (
                <Card key={index} className="p-4 md:p-6 text-center border border-[var(--color-border)]">
                  <div 
                  ...
                  <div className="text-2xl md:text-3xl font-bold mb-1" style={{ color: 'var(--color-text)' }}>
                    {stat.value}
                  </div>
                  <div className="text-sm" style={{ color: 'var(--color-text-light)' }}>{stat.label}</div>
                </Card>
              ))}
            </motion.div>
                ...
                <motion.div variants={itemVariants}>
                  <Card className="p-6 md:p-8 border border-[var(--color-border)]">
                    ...
                    <p className="leading-relaxed mb-6" style={{ color: 'var(--color-text-light)' }}>
                      {shelter.description || `${shelter.name} is dedicated to rescuing and rehoming animals in need. Our mission is to provide a safe haven for abandoned and surrendered pets while finding them loving forever homes.`}
                    </p>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-sm" style={{ color: 'var(--color-text-light)' }}>
                            {shelter.establishedDate ? new Date(shelter.establishedDate).getFullYear() : 'N/A'}
                          </div>
                        </div>
                      </div>
                      <div className="flex items-start gap-3">
                  ...
                  <Card className="p-6 md:p-8 border border-[var(--color-border)]">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                      ...
                      <div className="flex gap-2 overflow-x-auto pb-2 sm:pb-0">
                        {(['all', 'dogs', 'cats', 'other'] as const).map(tab => (
                            <button 
                                key={tab} 
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {filteredPets.length > 0 ? filteredPets.slice(0, 4).map((pet, index) => (
                        <motion.div 
                            key={pet.id} 
                              ...
                              <div className="group rounded-xl overflow-hidden hover:shadow-md transition-shadow bg-[var(--color-surface)]">
                                <div className="aspect-[4/3] relative overflow-hidden">
                                  ...
                                  <div className="absolute top-3 right-3">
                                    <Badge variant={pet.adoptionStatus === 'available' ? 'success' : pet.adoptionStatus === 'pending' ? 'warning' : 'neutral'}>
                                      {pet.adoptionStatus}
                                    </Badge>
                                  </div>
                                </div>
                        ...
                        <div className="col-span-full text-center py-12" style={{ color: 'var(--color-text-light)' }}>
                    ...
                    <div className="space-y-1 text-sm bg-[var(--color-surface)] p-2 rounded-xl">
                      {['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'].map((day) => {
                         ...
                         return (
                            <div 
                              key={day} 
                              className={`flex justify-between items-center p-2 rounded-lg transition-colors ${isToday ? 'bg-[var(--color-primary)]/10 font-semibold' : ''}`}
                            >
                              <span className="capitalize" style={{ color: isToday ? 'var(--color-primary)' : 'var(--color-text-light)' }}>
                                {day} {isToday && '(Today)'}
                              </span>
                              <span 
                                className={!schedule || schedule.closed ? 'text-red-500' : ''}
                                style={{ color: !schedule || schedule.closed ? undefined : 'var(--color-text)' }}
                              >
                                {schedule && !schedule.closed ? (
                                  `${formatTime(schedule.open)} - ${formatTime(schedule.close)}`
                                ) : (
                                  <span className="text-xs px-2 py-0.5 rounded-full bg-red-100 text-red-600">Closed</span>
                                )}
                              </span>
                            </div>
                         );
