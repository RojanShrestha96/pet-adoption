...
import { 
  ArrowLeft, ArrowRight, Save, PawPrint, Check, 
  Upload, X, FileText, Syringe, Cpu, Scissors, Bug, 
  Stethoscope, Heart, Calendar, AlertCircle, CheckCircle2,
  Image as ImageIcon
} from 'lucide-react';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.medicalDocuments.length > 0) {
        ...
        const docResponse = await fetch('http://localhost:5000/api/upload/documents', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`
          },
      ...
      showToast('Pet submitted for review! 🐾 Admin will approve it shortly.', 'success');
        ...
        <header className="bg-white border-b border-gray-200 sticky top-0 z-20 px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Add New Pet</h1>
              ...
              {STEPS.map((step) => (
                ...
                >
                  ...
                  <span className={`hidden sm:block text-xs mt-1 ${
                    step.id === currentStep ? 'text-[var(--color-primary)] font-medium' : 'text-gray-500'
                  }`}>
                    {step.title}
                  </span>
                </button>
          ...
          <div className="max-w-3xl mx-auto">
                ...
                >
                  <Card className="p-6">
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <div>
                        <h2 className="text-xl font-bold text-gray-900">Pet Photos</h2>
                        <p className="text-sm text-gray-500">Upload high-quality images (max 5)</p>
                      </div>
                    </div>
                    <DragDropImageUpload 
                  ...
                  <Card className="p-6">
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">Tell us about the pet</p>
                      </div>
                    </div>
                ...
                >
                  <Card className="p-6">
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <div>
                        <h2 className="text-xl font-bold text-gray-900">Physical Details</h2>
                        <p className="text-sm text-gray-500">Age, size, and characteristics</p>
                      </div>
                    </div>
                    ...
                    <div className="mt-6">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {['Friendly', 'Playful', 'Calm', 'Energetic', 'Shy', 'Affectionate', 'Independent', 'Loyal', 'Curious', 'Gentle'].map(trait => (
                          <button
                            key={trait}
                            type="button"
                  ...
                  <Card className="p-6">
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">Health status and documentation</p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {/* Vaccination */}
                      <div 
                        onClick={() => handleInputChange('isVaccinated', !formData.isVaccinated)}
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className={`p-2 rounded-lg ${formData.isVaccinated ? 'bg-green-500' : 'bg-gray-200'}`}>
                            <Syringe className={`w-5 h-5 ${formData.isVaccinated ? 'text-white' : 'text-gray-500'}`} />
                          </div>
                          <div>
                            <p className="font-medium text-gray-900">Vaccinated</p>
                            <p className="text-xs text-gray-500">Up to date vaccinations</p>
                          </div>
                          {formData.isVaccinated && <CheckCircle2 className="w-5 h-5 text-green-500 ml-auto" />}
                        </div>
                      </div>

                      {/* Microchipped */}
                      <div 
                        onClick={() => handleInputChange('isMicrochipped', !formData.isMicrochipped)}
                        className={`p-4 rounded-xl border-2 cursor-pointer transition-all ${
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className={`p-2 rounded-lg ${formData.isMicrochipped ? 'bg-green-500' : 'bg-gray-200'}`}>
                            <Cpu className={`w-5 h-5 ${formData.isMicrochipped ? 'text-white' : 'text-gray-500'}`} />
                          </div>
                          <div>
                            <p className="font-medium text-gray-900">Microchipped</p>
                            <p className="text-xs text-gray-500">Has identification chip</p>
                          </div>
                          {formData.isMicrochipped && <CheckCircle2 className="w-5 h-5 text-green-500 ml-auto" />}
                        </div>
                      </div>

                      {/* Neutered/Spayed */}
                      <div 
                        onClick={() => handleInputChange('isNeutered', !formData.isNeutered)}
                        className={`p-4 rounded-xl border-2 cursor-pointer transition-all ${
                          formData.isNeutered
                            ? 'border-green-500 bg-green-50'
                            : 'border-gray-200 hover:border-gray-300'
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className={`p-2 rounded-lg ${formData.isNeutered ? 'bg-green-500' : 'bg-gray-200'}`}>
                            <Scissors className={`w-5 h-5 ${formData.isNeutered ? 'text-white' : 'text-gray-500'}`} />
                          </div>
                          <div>
                            <p className="font-medium text-gray-900">Neutered/Spayed</p>
                            <p className="text-xs text-gray-500">Sterilization complete</p>
                          </div>
                          {formData.isNeutered && <CheckCircle2 className="w-5 h-5 text-green-500 ml-auto" />}
                        </div>
                      </div>

                      {/* Dewormed */}
                      <div 
                        onClick={() => handleInputChange('isDewormed', !formData.isDewormed)}
                        className={`p-4 rounded-xl border-2 cursor-pointer transition-all ${
                          formData.isDewormed
                            ? 'border-green-500 bg-green-50'
                            : 'border-gray-200 hover:border-gray-300'
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className={`p-2 rounded-lg ${formData.isDewormed ? 'bg-green-500' : 'bg-gray-200'}`}>
                            <Bug className={`w-5 h-5 ${formData.isDewormed ? 'text-white' : 'text-gray-500'}`} />
                          </div>
                          <div>
                            <p className="font-medium text-gray-900">Dewormed</p>
                            <p className="text-xs text-gray-500">Deworming treatment done</p>
                          </div>
                          {formData.isDewormed && <CheckCircle2 className="w-5 h-5 text-green-500 ml-auto" />}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-4">
                        ...
                        <input
                          ...
                          onKeyDown={e => {
                            if (e.key === 'Enter') {
                              ...
                              if (value && !formData.otherConditions.includes(value)) {
                                handleInputChange('otherConditions', [...formData.otherConditions, value]);
                                input.value = '';
                              }
                            }
                          }}
                            ...
                            >
                              ...
                              <button
                                ...
                                onClick={() => {
                                  handleInputChange(
                                    'otherConditions',
                                    formData.otherConditions.filter((_, i) => i !== idx)
                                  );
                                }}
                      ...
                      <p className="text-xs text-gray-500 mt-2">E.g., Diabetes, Arthritis, Blind, Deaf, Special diet required</p>
                    </div>
                  </Card>
                  ...
                  <Card className="p-6">
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">Upload vaccination records, vet reports, etc.</p>
                      </div>
                    </div>
                  ...
                  <Card className="p-6">
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">Check all information before submitting</p>
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-50 rounded-2xl p-6">
                      <div className="flex gap-6">
                        ...
                        <div className="flex-1">
                          <h3 className="text-2xl font-bold text-gray-900">{formData.name || 'Pet Name'}</h3>
                          <p className="text-gray-500 capitalize">{formData.breed || 'Breed'} • {formData.species || 'Species'}</p>
                          ...
                          <div className="flex flex-wrap gap-2 mt-4">
                            ...
                            {formData.otherConditions.map((condition, idx) => (
                              <span key={idx} className="flex items-center gap-1 text-orange-600 text-sm">
                        ...
                        <div className="mt-4 pt-4 border-t border-gray-200">
                          <div className="flex items-center gap-2 text-[var(--color-primary)]">
                            ...
                            <span className="text-sm font-medium">
                              {formData.medicalDocuments.length} document(s) attached
                            </span>
                          </div>
                        </div>
                      )}
                    ...
                    <div className="mt-6 pt-6 border-t border-gray-200">
                      <p className="text-sm font-medium text-gray-700 mb-3">Need to make changes?</p>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600 mt-1">
                            After submission, your pet listing will be reviewed by our admin team. 
            ...
            <div className="flex items-center justify-between mt-8 pt-6 border-t border-gray-200">
              ...
              {currentStep < STEPS.length ? (
                ...
                >
                  {isSubmitting ? <LoadingSpinner size="sm" color="white" /> : 'Add Pet'}
                </Button>
              )}
            </div>
          </div>
